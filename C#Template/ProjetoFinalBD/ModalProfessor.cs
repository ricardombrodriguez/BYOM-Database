    ...
    public partial class ModalProfessor : Form
    {
        ...
        private void btnAdicionarProfessor_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cmd.CommandText = "SELECT COUNT(*) FROM PROJETO.Professor WHERE email = @email AND disabled = 0";
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@email", email.Text);
                cmd.Connection = cn;
                int professorExistente = Convert.ToInt32(cmd.ExecuteScalar());

                //se existir, pode ser adicionado
                if (professorExistente == 1)
                {

                    if (CriarProfessor.createProfessorCadeira == true)
                    {

                        SqlCommand cm = new SqlCommand();
                        cm.CommandText = "SELECT COUNT(*) FROM PROJETO.ProfessorCadeira WHERE cadeira = @cadeira AND professor = @professor";
                        cm.Parameters.Clear();
                        cm.Parameters.AddWithValue("@cadeira", Cadeira.cadeiraAtual.Id);
                        cm.Parameters.AddWithValue("@professor", email.Text);
                        cm.Connection = cn;
                        int professorNaCadeira = Convert.ToInt32(cm.ExecuteScalar());
                        ...
                        } else
                        {
                            ...
                            try
                            {
                                ...
                                CriarProfessor.instCadeira.showProfessores();
                                MessageBox.Show("Professor(a) " + email.Text + " foi adicionado(a) à cadeira.", "SUCCESS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                this.Hide();
                            }
                            catch (Exception ex)
                            {
